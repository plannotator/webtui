...
import { terminalOutputPrefersRenderRefresh } from '../src/browser/index.js'
...
describe('terminalOutputPrefersRenderRefresh', () => {
  ...
  it('detects ASCII ANSI background SGR output before the non-ASCII fast path', () => {
    ...
    expect(terminalOutputPrefersRenderRefresh('\x1b[104m bright selected block \x1b[0m')).toBe(
      true
    )
  })

  it('does not refresh for ordinary terminal output or ANSI controls alone', () => {
