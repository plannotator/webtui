...
export function getTerminalWebglAutoDecision(): TerminalWebglAutoDecision {
  ...
  if (!rendererInfo.hasRendererInfo) {
    // Linux software GL can leave WebGL alive while corrupting terminal glyphs;
    // without renderer identity, auto mode cannot distinguish hardware from software GL.
    cachedDecision = {
      allowWebgl: false,
